...
Prerequisites
•	In order to follow along with this tutorial, you should have a Node.js server environment and npm installed on your computer. Learn about Node.js and npm and how to install them on Mac/Windows.
...
Input the following command to install all the dependencies:
...
npm inatall debug@2.6.9 --save-dev 
...
Create a Test directory and add a test.js file in this new directory. We will use the built-in Node.js assert module to test if true is equal to true. Since it is, the test should pass.
test.js
...
test.js
const assert = require('assert');
test.js is the file we will use for testing throughout the creation of our app. Let’s create one additional file: operations.js for our arithmetic and validation functions, and our existing calc.js file is for our app itself. We want to keep our files separate so they don’t get too long and complex. Below is our list of files and folders.
•	calc.js
•	node_modules
•	operations.js
•	package-lock.json
...
Let’s start with addition. We will write a test to calculate the sum of two numbers that we know will be true. The below code is testing if 1 plus 3equals 4 in the add() function.
test.js
const assert = require('assert');
...
Note: This function is using ES6 arrow functions and implicit returns. Read more about the different ways to write a function in JavaScript for increased clarity.
...
Looking back at our function, it requires that both parameters must be NaNto fail. We want it to fail even if only one parameter is NaN, so we will change && to ||.
operations.js
const validateNumbers = (x, y) => {
  if (isNaN(x) || isNaN(y)) {
    return false;
...
If all input is valid and correct, we will want to move forward with the process and run the respective mathematical operations that we created earlier. We will use a switch statement to execute code based on the three possible choices, and output the result of the operation. If an invalid choice is made, the default code block will run, telling the user to start over.
calc.js
if (!operations.validateNumbers(x, y)) {
  console.log('Only numbers are allowed! Please restart the program.');
} else {
...
Ensure that the build stage runs successfully. If the stage fails, check the build stage logs, make any necessary corrections, and then re-run the stage.
...
If you jump back to your delivery pipeline you should be able to see the stages running automatically. When the deploy succeeds your app will now display a new message when you visit your route. Which should fail!!
